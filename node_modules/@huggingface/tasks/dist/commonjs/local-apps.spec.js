...
curl -X POST "http://localhost:8000/v1/chat/completions" \\
    ...
    (0, vitest_1.it)("unsloth tagged model", async () => {
        const { displayOnModelPage, deeplink } = local_apps_js_1.LOCAL_APPS.unsloth;
        const model = {
        ...
        (0, vitest_1.expect)(displayOnModelPage(model)).toBe(true);
        (0, vitest_1.expect)(deeplink(model, undefined).href).toBe("unsloth://open_from_hf?model=some-user%2Fmy-unsloth-finetune");
    });
    (0, vitest_1.it)("unsloth namespace gguf model", async () => {
        ...
        (0, vitest_1.expect)(displayOnModelPage(model)).toBe(true);
        (0, vitest_1.expect)(deeplink(model, undefined).href).toBe("unsloth://open_from_hf?model=unsloth%2FLlama-3.2-3B-Instruct-GGUF");
        (0, vitest_1.expect)(deeplink(model, "Llama-3.2-3B-Instruct-UD-Q4_K_XL.gguf").href).toBe("unsloth://open_from_hf?model=unsloth%2FLlama-3.2-3B-Instruct-GGUF&file=Llama-3.2-3B-Instruct-UD-Q4_K_XL.gguf");
    });

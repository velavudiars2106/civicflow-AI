...
exports.FalAIImageSegmentationTask = exports.FalAIAudioToAudioTask = exports.FalAITextToAudioTask = exports.FalAITextToSpeechTask = exports.FalAIAutomaticSpeechRecognitionTask = exports.FalAIImageTextToVideoTask = exports.FalAIImageToVideoTask = exports.FalAITextToVideoTask = exports.FalAIImageTextToImageTask = exports.FalAIImageToImageTask = exports.FalAITextToImageTask = exports.FAL_AI_SUPPORTED_BLOB_TYPES = void 0;
/**
...
function dropEndpointSegmentOnDirectCalls(url) {
    ...
    if (urlObj.origin === config_js_1.HF_ROUTER_URL) {
        return url;
    }
...
class FalAIAutomaticSpeechRecognitionTask extends FalAITask {
    ...
    async getResponse(response) {
        ...
        if (typeof text !== "string") {
            throw new errors_js_1.InferenceClientProviderOutputError(`Received malformed response from Fal.ai Automatic Speech Recognition API: expected { text: string } or { output: string } format, got instead: ${JSON.stringify(response)}`);
        }
        const output = { text };
        const chunks = Array.isArray(res.chunks)
            ...
            : Array.isArray(res.segments)
                ? res.segments
                    ...
                    .map((s) => ({ text: s.text, timestamp: [s.start ?? 0, s.end ?? 0] }))
                : [];
        if (chunks.length > 0) {
...
class FalAITextToAudioTask extends FalAiQueueTask {
    task;
    constructor() {
        super("https://queue.fal.run");
    ...
    async getResponse(response, url, headers, _outputType, signal) {
        ...
        if (typeof audio !== "object" || !audio || typeof audio.url !== "string" || !(0, isUrl_js_1.isUrl)(audio.url)) {
            throw new errors_js_1.InferenceClientProviderOutputError(`Received malformed response from Fal.ai text-to-audio API: expected { audio_file: { url: string } } or { audio: { url: string } } result format, got instead: ${JSON.stringify(result)}`);
        }
        const audioResponse = await fetch(audio.url, { signal });
        if (!audioResponse.ok) {
            throw new errors_js_1.InferenceClientProviderApiError(`Failed to fetch audio from ${audio.url}: ${audioResponse.statusText}`, { url: audio.url, method: "GET" }, {
...
exports.FalAITextToAudioTask = FalAITextToAudioTask;
class FalAIAudioToAudioTask extends FalAiQueueTask {
    task;
    constructor() {
        super("https://queue.fal.run");
    ...
    async preparePayloadAsync(args) {
        const blob = "data" in args && args.data instanceof Blob ? args.data : "inputs" in args ? args.inputs : undefined;
        if (!(blob instanceof Blob)) {
            throw new errors_js_1.InferenceClientInputError(`Expected a Blob input for audio-to-audio with provider fal-ai, got ${typeof blob}.`);
        }
        return {
            ...("data" in args ? (0, omit_js_1.omit)(args, "data") : (0, omit_js_1.omit)(args, "inputs")),
            audio_url: await buildFalAiAudioDataUrl(blob),
    ...
    async getResponse(response, url, headers, _outputType, signal) {
        ...
        if (typeof result !== "object" ||
            ...
            !(0, isUrl_js_1.isUrl)(result.audio.url)) {
            throw new errors_js_1.InferenceClientProviderOutputError(`Received malformed response from Fal.ai audio-to-audio API: expected { audio: { url: string } } result format, got instead: ${JSON.stringify(result)}`);
        }
        const audioResponse = await fetch(result.audio.url, { signal });
        if (!audioResponse.ok) {
            throw new errors_js_1.InferenceClientProviderApiError(`Failed to fetch audio from ${result.audio.url}: ${audioResponse.statusText}`, { url: result.audio.url, method: "GET" }, {
                requestId: audioResponse.headers.get("x-request-id") ?? "",
                status: audioResponse.status,
                body: await audioResponse.text(),
            });
        ...
        return [
            {
                blob: (0, base64FromBytes_js_1.base64FromBytes)(audioBytes),
...
exports.FalAIAudioToAudioTask = FalAIAudioToAudioTask;
